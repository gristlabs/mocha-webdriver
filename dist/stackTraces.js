...
exports.stackWrapDriverMethods = exports.stackWrapOwnMethods = exports.stackWrapFunc = void 0;
/**
 ...
 */
function stackWrapFunc(fn) {
    if (!process.env.MOCHA_WEBDRIVER_STACKTRACES) {
        return fn;
    }
    return wrap(fn);
}
exports.stackWrapFunc = stackWrapFunc;
/**
 ...
 */
function stackWrapOwnMethods(_obj) {
    if (!process.env.MOCHA_WEBDRIVER_STACKTRACES) {
        return _obj;
    }
    const obj = _obj;
...
exports.stackWrapOwnMethods = stackWrapOwnMethods;
...
function wrap(fn) {
    const wrapped = function () {
        ...
        if (!ret || typeof ret.catch !== 'function' || ret instanceof index_1.WebDriver) {
            return ret;
        }
        // Otherwise, tack on to the stack the stack trace from origErr, with cleaning.
        const ret2 = ret.catch((err) => { throw cleanStack(err, origErr); });
        // And if it's a WebElementPromise, make sure we still return one.
        return (ret instanceof index_1.WebElementPromise) ? new index_1.WebElementPromise(ret.getDriver(), ret2) : ret2;
    };
    wrapped.origFunc = fn;
    return wrapped;
}
// Combine err.stack with origErr.stack, with an attempt to make it readable and helpful.
function cleanStack(err, origErr) {
...
function stackWrapDriverMethods() {
    stackWrapOwnMethods(index_1.WebDriver.prototype);
    stackWrapOwnMethods(index_1.WebElement.prototype);
    stackWrapOwnMethods(index_1.WebElementPromise.prototype);
}
exports.stackWrapDriverMethods = stackWrapDriverMethods;
//# sourceMappingURL=stackTraces.js.map